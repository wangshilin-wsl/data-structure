...
/*力扣：1620. 网络信号最好的坐标
        ...
        0 <= xi, yi, qi <= 50
        1 <= radius <= 50
        思路：由于数据不多直接暴力法，先找到最大的x和y，并从0开始到最大的x,y循环
        */
public class BestCoordinate {
    public static void main(String[] args) {
        new BestCoordinate().bestCoordinate(new int[][]{{1,2,5},{2,1,7},{3,1,9}}, 2);
    }
    ...
    public int distance(int[] tow, int x, int y, int radius) {
        int dis = (int)Math.pow((x - tow[0]), 2) + (int)Math.pow((y - tow[1]), 2);
        if (dis > radius * radius) {
            return 0;
        }
        double sqrt = Math.sqrt(dis);
        return (int) Math.floor(tow[2] / (1 + sqrt));
    }
}
