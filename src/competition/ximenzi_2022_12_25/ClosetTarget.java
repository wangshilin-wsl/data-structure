...
/*6269. 到目标字符串的最短距离 显示英文描述
        ...
        思路：可以从startIndex向两边发散求*/
public class ClosetTarget {
    public static void main(String[] args) {
        System.out.println(new ClosetTarget().closetTarget(new String[]{"hello", "i", "am", "leetcode", "hello"}, "hello", 1));
    }
    public int closetTarget(String[] words, String target, int startIndex) {
        ...
        for (int i = 0; i < n; i++) {
            if (words[i].equals(target)) {
                ...
                min = Math.min(min, Math.min(temp, n - temp));
            }
        }
